...
class ANN(nn.Module):
  def __init__(self, input_dim: int=5, hidden_dim: list=[128, 128, 64, 32], activation: str="relu", use_dropout: bool=False, drop_ratio: float=0.3):
    super().__init__()
    ...
    output_layer = [nn.Linear(dims[-1], 1)] # Relu는 항상 양수 값일 때 주는 방법
